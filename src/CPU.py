...
class CPU:
    ...
    def SEU(self, chance=0.01):
        upset_attempt = random.uniform(0, 1)
        if upset_attempt < chance:
            upset_register_num = random.randint(0, 4)
            upset_register = self.registers[upset_register_num]
            upset_bit = random.randint(0, 15)
            upset_register.set_value(upset_register.get_value() + upset_values[upset_bit])
            print("\nSINGLE EVENT UPSET! Register:", upset_register_num + 1, "Bit:", upset_bit + 1)
